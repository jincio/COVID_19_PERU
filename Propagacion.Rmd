...
f6=data%>%
  ...
  geom_hline(yintercept=656, linetype="dashed", 
             color = "orange", size=1)+
  geom_hline(yintercept=820, linetype="dashed", 
             color = "blue", size=1)+
  annotate(geom="text", 
  ...
  annotate(geom="text", 
           label="Capacidad al 23/04", x=max(data$Dia), 
           y=656, vjust=-1,hjust = 2) +
  annotate(geom="text", 
           label="Capacidad actual: 820", x=max(data$Dia), 
           y=726, vjust=-1,hjust = 2) +
  theme_minimal() +
  theme(legend.position="bottom")
```
