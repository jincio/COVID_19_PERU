...
```{r positivos}
grafico2 <- casos_diarios(positivos,"Junin",mediamovil = 3)[[2]]+ theme_bw() + scale_fill_manual(values = "#00AFBB") +
...
**Región Puno**
```{r positivos2}
grafico2 <- casos_diarios(positivos,"Puno",mediamovil = 3)[[2]]+ theme_bw() + scale_fill_manual(values = "#00AFBB") +
  scale_color_manual(values = "#FC4E07")
print(grafico2)
```
...
r0 <- resultados[["R0"]]
ggplot() +
  geom_line(aes(x = c(1:nrow(r0)), y = r0),
            colour = "darkblue") + labs(x = "Dia" , y = "R0") + 
  scale_x_continuous(breaks = seq(1,nrow(r0),15), labels = seq(1,nrow(r0),15)) +
  geom_hline(yintercept = 1, colour = "red") + labs(title = "Evolución del R0 a nivel Nacional",
                                                    x = "Dia del inicio de la pandemia")+
  theme_bw()
