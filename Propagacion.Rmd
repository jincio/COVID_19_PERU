...
f1=data %>%
  ...
  geom_bar(aes(y = Nuevos_Positivos, fill = "Nuevos"), stat = "identity", alpha=.5)+
  #geom_label(aes(Dia, Nuevos_Positivos, label = Nuevos_Positivos), vjust = 0.5,label.size = 0.10)+
  geom_text_repel(
    aes(x=Dia,y=Nuevos_Positivos,label = Nuevos_Positivos), 
    vjust = -0.3,
    size = 3.1,
    box.padding = 0.05)+
  ...
  geom_text_repel(
    aes(x=Dia,y=Positivos,label = Positivos), 
    vjust = -0.3,
    size = 3.1,
    box.padding = 0.05)+
...
f4=data %>%
  ...
  #geom_label(aes(Dia, Nuevos_Positivos, label = Nuevos_Positivos), vjust = 0.5,label.size = 0.10)+
  geom_text_repel(
    aes(x=Dia,y=Pruebas_dia,label = Pruebas_dia), 
    vjust = -0.3,
    size = 3.1,
    box.padding = 0.05)+
  ...
  geom_text_repel(
    aes(x=Dia,y=Total_Pruebas,label = Total_Pruebas), 
    vjust = -0.3,
    size = 2.1,
    box.padding = 0.05)+
...
f4_1=data %>%
  ...
  geom_bar(aes(y = Pruebas_dias, fill = "Nuevos"), stat = "identity", alpha=.5)+
  geom_text_repel(
    aes(x=Dia,y=Pruebas_dias,label = Pruebas_dias), 
    vjust = -0.3,
    size = 2.1,
    box.padding = 0.05)+
  geom_line(aes(y = Total_Pruebas, col = "Acumulados"), size=1) +
...
f6=data%>%
  ...
  geom_hline(yintercept=500, linetype="dashed", 
             color = "blue", size=1)+
  annotate(geom="text", 
           label="Capacidad actual: 276", x=max(data$Dia), 
           y=276, vjust=-1,hjust = 1)+
  annotate(geom="text", 
           label="Capacidad deseada: 500", x=max(data$Dia), 
           y=500, vjust=-1,hjust = 1) +
  theme_minimal() +
  theme(legend.position="bottom")
```
...
panel<- ggarrange(f1_1,
          ncol=1,nrow=1,
          font.label = list(size = 10, color = "black", face = "bold", family = NULL))
          annotate_figure(panel,
                top = text_grob("Número de casos positivos de pacientes COVID-19,\n Perú (Moleculares+PR)", color = "#474785", face = "bold", size = 14),
                bottom = text_grob("Fuente: MINSA. Ver (https://jincio.github.io/COVID_19_PERU/Propagacion.html)", color = "black", hjust = 1, x = 1, face = "italic", size = 8)
)
...
panel<- ggarrange(f4,
          ...
          font.label = list(size = 10, color = "black", face = "bold", family = NULL))
          annotate_figure(panel,
                top = text_grob("Número de pruebas moleculares analizadas COVID-19 \n (acumulados y nuevos), Perú", color = "#474785", face = "bold", size = 14),
                bottom = text_grob("Fuente: MINSA. Ver (https://jincio.github.io/COVID_19_PERU/Propagacion.html)", color = "black", hjust = 1, x = 1, face = "italic", size = 8)
                #left = text_grob("Figure arranged using ggpubr", color = "green", rot = 90),
                #fig.lab = "Figure 1", fig.lab.face = "bold"
)
...
# Panel para publicacion
panel<- ggarrange(f4_1,
          ncol=1,nrow=1,
          font.label = list(size = 10, color = "black", face = "bold", family = NULL))
          annotate_figure(panel,
                top = text_grob("Número de pruebas moleculares y rápidas \n analizadas COVID-19 (acumulados y nuevos), Perú", color = "#474785", face = "bold", size = 14),
                bottom = text_grob("Fuente: MINSA. Ver (https://jincio.github.io/COVID_19_PERU/Propagacion.html)", color = "black", hjust = 1, x = 1, face = "italic", size = 8)
                #left = text_grob("Figure arranged using ggpubr", color = "green", rot = 90),
                #fig.lab = "Figure 1", fig.lab.face = "bold"
)
...
El ministro en declaraciones a medios de prensa y en la conferencia del día 27 de Marzo manifestó que las camas UCI disponibles para los afectados por Coronavirus es de 276 unidades. En diferentes medios mencionó que el gobierno está haciendo el esfuerzo para aumentar la capacidad a 500 unidades. 
