...
data3 = data2 %>%
  ...
  mutate(Departamento = ifelse(REGION=="CALLAO","LIMA",
                             REGION)) %>%
...
m1 = departamento %>%
  ...
  ggplot() +
    ...
    scale_fill_manual(values = c("grey80", colorRampPalette(colors = c("#FFF5F0","#FB6A4A", "#67000D"))(13))) + #reemplazar numero por n de levels en factor(casos) - 1
    #geom_sf_text(aes(label = casos), colour = "black")+
    scale_x_discrete()+
    labs(title = paste0("Número de casos positivos por departamento"),
