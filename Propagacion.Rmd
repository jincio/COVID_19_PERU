...
Código adaptado del post de Gabriel Carrasco Escobar (http://gcarrasco.rbind.io/blog/covid19_viz/)
...
f2=data %>%
  ...
  labs(y = "proportion", fill = "Resultado",
       title = paste0("Proporcion de positivos del total \nde muestras analizadas hasta: ", format(as.Date(max(data$Dia)),"%d/%m"))) +
  theme_bw() +
  theme(legend.position="bottom")
...
departamento%>%
  ...
  labs(title = paste0("Proporcion de positivos del total \nde muestras analizadas hasta: ", format(as.Date(max(data2$Fecha)),"%d/%m")),
